...
const LandingPage = () => {
  return (
    <div className="relative flex min-h-screen flex-col overflow-hidden bg-gradient-to-br from-surface via-[#070a1d] to-[#02030c] text-foreground">
      <div className="pointer-events-none absolute inset-x-0 top-0 h-[50vh] bg-[radial-gradient(circle_at_top,_rgba(124,92,255,0.45),_transparent_65%)]" />
      <div className="pointer-events-none absolute inset-0 -z-10 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-40" />

      <div className="relative mx-auto flex w-full max-w-6xl flex-1 flex-col px-6 py-16 lg:px-12">
        <header className="flex items-center justify-between">
          ...
          <Link
            to="/dashboard"
            className="hidden items-center gap-2 rounded-full border border-white/10 bg-white/10 px-4 py-2 text-sm font-medium text-muted transition hover:border-accent hover:bg-accent/20 hover:text-white sm:flex"
          >
            Enter dashboard
        ...
        <main className="mt-20 grid flex-1 gap-14 lg:grid-cols-[1.2fr_1fr]">
          <div className="space-y-10">
            ...
            <p className="max-w-xl text-lg text-muted">
              Launch the Command Center to supervise telemetry, stream the onboard camera, dispatch manual maneuvers, and triage alerts—all wrapped in a beautifully responsive interface designed for demos.
            </p>
          ...
          <div className="relative">
            <div className="absolute inset-0 rounded-[40px] bg-gradient-to-br from-accent/30 via-transparent to-sky-500/10 blur-3xl" />
            <div className="relative rounded-[32px] border border-white/10 bg-[#0d1023]/80 p-8 shadow-card backdrop-blur-lg">
              <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-sm text-muted">
                <p className="mb-3 text-xs uppercase tracking-[0.35em] text-muted">Demo Ready</p>
                <p className="font-display text-2xl text-foreground">Pair your Pi, power the Windows AI bridge, and press launch.</p>
              </div>
              <ul className="mt-6 space-y-4 text-sm text-muted">
                <li>• Raspberry Pi supervisor bundles the camera stream + Chirpy voice.</li>
                <li>• Windows supervisor exposes REST endpoints for the React dashboard.</li>
                <li>• One command center unifies monitoring, control, and alerts.</li>
              </ul>
            </div>
          </div>
        </main>
        ...
        <section id="features" className="mt-24 grid gap-6 md:grid-cols-3">
          {features.map(feature => (
            <div
              key={feature.title}
              className="rounded-3xl border border-white/5 bg-[#0d1023]/70 p-6 shadow-card backdrop-blur-lg"
            >
